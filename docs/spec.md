...
### 6.5 利用地点での実測（流山おおたかの森駅 35.8709 / 139.9256、標高約15m）
...
流山おおたかの森を中心に半径 60NM の実データを 10 秒おきに 3 回取得し、航跡と手順の折れ線を照合した。
...
2. 羽田の南風運用で使われる LDA（RWY22/23）が取れたことで、流山から見える低高度の到着機を説明できるようになった
...
| 機体写真 | **planespotters.net の写真 API を使う**（撮影者名と写真ページのリンクを返す）。撮影者名とリンクを必ず添え、取得できなければ写真を出さない。呼び出しの User-Agent には、先方の規約どおり連絡先（URL かメール）を入れる。adsbdb が返す写真（airport-data.com）は**撮影者名が取れない**ので使わない |
...
| 2 | 地図の機体アイコンがすべて Tab の停止点になる | 一覧をキーボード操作の主役にする方針なので、**アイコンは Tab 順から外す**（マーカーを非フォーカスにする）。次の修正で対応 |
...
| 5 | 機体写真の差し替え | Q12 の決定にともない、planespotters.net へ切り替える（連絡先入り User-Agent が必要。論点 J） |
...
| Q10 | 利用地点 | **流山おおたかの森駅**（35.8709 / 139.9256、標高約15m）。カバレッジ確認済み（6.5） |
...
| J | **写真 API に入れる連絡先** | planespotters の規約で、User-Agent に連絡先の URL かメールが必要。何を入れるか（公開リポジトリの URL か、メールアドレスか）。連絡先を出したくなければ、写真をやめてリンクのみにする |

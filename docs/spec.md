...
| 5 | ~~機体写真の差し替え~~ | **対応済み**（2026-09-16）。planespotters.net へ切り替え、撮影者名を写真ページへのリンクにした。実機で表示を確認（例: 「写真: Hugo Lam」→ 該当写真のページ）。**ただし User-Agent に入れた連絡先 URL のリポジトリはまだ作っていない**（論点 J） |
...
| J | 写真 API に入れる連絡先 | **公開リポジトリの URL に決定**し、`https://github.com/kwanta08/flightboard` を User-Agent に入れた（`src/server/providers/provider.ts`）。**残件: このリポジトリをまだ作っていない**ので、公開して URL を実在させる必要がある |

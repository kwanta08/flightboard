...
describe("AC-P2-03: 対向端どうしの距離と公示滑走路長", () => {
  ...
  // 下の検査はすべて LENGTH_TOLERANCE を通して書いてあるので、取り込み側の定数を緩めると
  // 不一致が検出から外れ、生成物のヘッダも「なし」になって全部緑のまま通ってしまう。
  // 受入基準が固定しているのは ±10% なので、値そのものをここで押さえる（AC-P2-02 の 7° と同じ扱い）
  it("許容は公示滑走路長の ±10% に固定（取り込み側の定数を緩めて黙らせられない）", () => {
    expect(LENGTH_TOLERANCE).toBe(0.1);
  });
